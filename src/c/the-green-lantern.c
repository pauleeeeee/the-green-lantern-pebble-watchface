...
// Layout boundaries
#define TIME_RING PBL_IF_ROUND_ELSE(26, 20)
#define DOT_RING PBL_IF_ROUND_ELSE(6, 20)
#define DATE_RING PBL_IF_ROUND_ELSE(65, 20)


// Persistent Keys
#define SHOW_SECONDS 33
...
static void draw_main_clock(Layer *layer, GContext *ctx) {
  ...
  for(int i = 0; i < 13; i++) {
    ...
    graphics_context_set_fill_color(ctx, GColorBlack);
    GPoint tick = gpoint_from_polar(inner_hour_frame, GOvalScaleModeFitCircle, DEG_TO_TRIGANGLE(get_angle_for_hour(i)));
...
static void draw_clock_hands(Layer *layer, GContext *ctx) {
  ...
  // if (persist_exists(SHOW_SECONDS) && persist_read_bool(SHOW_SECONDS)) {
  ...
  int hour_hand_length = bounds.size.w / 3 - 10;
  ...
  GRect date = grect_centered_from_polar(date_ring, GOvalScaleModeFitCircle, DEG_TO_TRIGANGLE(get_angle_for_hour(3)), GSize(30,30));
  date.origin.x = 135;
  graphics_context_set_text_color(ctx, GColorBlack);
  graphics_draw_text(ctx, date_buffer, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD), date,
...
//   } else if (persist_exists(SHOW_SECONDS) && ! persist_read_bool(SHOW_SECONDS)) {
...
static void main_window_load() {
  ...
  lantern = gbitmap_create_with_resource(RESOURCE_ID_GL);
  lantern_layer = bitmap_layer_create(bounds);
  bitmap_layer_set_bitmap(lantern_layer, lantern);
  layer_add_child(window_layer, bitmap_layer_get_layer(lantern_layer));
